...
| Uç | İş |
|---|---|
| `GET /bridge/v1/health` | Köprü ve Logo sağlığı **ayrı** raporlanır |
...
`appsettings.json` içinde doldurulması zorunlu alanlar: `Bridge:ConnectionString`, `Bridge:FirmNumber`, `Bridge:PeriodNumber`, `Bridge:AllowedClientThumbprints`, `Kestrel` sunucu sertifikası. Eksik veya geçersiz bir değerle servis **açılmaz** — yanlış firma numarasıyla ayağa kalkan bir köprü, başka bir şirketin stok verisini portale taşır.
